...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit { padding: 12px 20px; background: #28a745; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; }
        ...
        .btn-delete { padding: 12px 20px; background: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; margin-left: 10px; }
    ...
    <div class="container">
        ...
        <form action="{{ route('produtos.update', $produto->id) }}" method="POST">
            ...
            <div class="form-group @error('quantidade') has-error @enderror">
                ...
                <input type="number" id="quantidade" name="quantidade" min="0" value="{{ old('quantidade', $produto->quantidade) }}" required>
                @error('quantidade')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="buttons">
                ...
                <button type="button" class="btn-delete" onclick="deletarProduto()">🗑️ Deletar</button>
            </div>
        </form>
        ...
        <script>
            function deletarProduto() {
                if (confirm('Tem certeza que deseja deletar este produto?')) {
                    document.getElementById('delete-form').action = "{{ route('produtos.destroy', $produto->id) }}";
